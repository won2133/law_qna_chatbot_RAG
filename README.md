...
* 법률과 관련된 질문과 그에 대한 답변이 있는 데이터
  * 이전에 수행했던 판례 추천 프로젝트(precedent_recommand)의 문제점 중 하나인 판례 추천 결과의 신뢰성이 낮다는 점을 개선하기 위해 이러한 데이터를 찾게 되었다.
...
## 유사도의 정확성 높이기
* 판례 추천 결과의 신뢰성이 낮다는 점을 개선하기 위해 정확도를 계산하고 높여야 했다. 우선 판례와 관련된 질문이 있는 데이터를 확보했으니, 정확도를 높이기 위해 다양한 형태소 분석기와 임베딩 모델 등을 비교하였다.
* 각 모델과 기법들을 비교하기 위해서, 수집한 데이터 중 하나를 답변으로 하는 또다른 질문과 답변 쌍이 필요했다. 
  * 그래서 질문을 역번역하여 새로운 질문과 응답쌍을 만들고 이 중 만 개의 샘플을 뽑아 비교하는 데 사용했다.
  * 결과로 출력된 다섯 개의 요약본 중 정답이 있는 것의 비율을 비교했다.
### 1. 형태소 분석기 선택
...
* 결과적으로 최종 선택된 모델과 기법들을 사용하였을 때, 가장 처음에 형태소 분석기를 선택할 때의 정답이 있을 확률 62.11%보다 20% 가량 향상된 것을 확인할 수 있었다.
  
## RAG 기반 챗봇 구현
* 이전 판례 추천 프로젝트(precedent_recommand)에서는 답변으로 판례 내용의 요약본을 제공해 사용자 질문과 다소 맞지 않은 내용이 나오기도 했고 이로 인해 사용자가 직접 판례를 읽어가며 답을 찾아야 하는 아쉬운 점이 있었다. 이를 보완하기 위해 단순히 추천하는 시스템이 아니라 유사하게 나온 내용을 참고하여 질문에 대한 답변을 생성하는 RAG 기반 챗봇을 구현하게 되었다.
### 1. 모델
* "EleutherAI/polyglot-ko-1.3b" 사용, 1에포크 학습
  * 토크나이저도 동일 토크나이저 사용
### 2. 학습 및 추론
* 프롬프트에 사용된 토큰 중 기존 토크나이저에 없었던 질문 토큰, 답변 토큰, context 토큰을 스페셜 토큰으로 만들어서 토크나이저에 추가하였다.
* 참고 목록의 kogpt2 챗봇 만드는 코드를 이용해서 직접 파이프라인 만들고 추론하였다. 
